...
const INITIAL_ORDERS: DhabaOrder[] = [
  { id: 'rc-1', customer: 'North Campus DU Students', locality: 'Hudson Lane', combo: 'Kashmiri Rajma Gravy + Jeera Rice + Sirka Pyaaz 🍛', price: 140, served: false },
  { id: 'rc-2', customer: 'Gurugram Cyber Hub Coders', locality: 'DLF Phase 3', combo: 'Dhaba Special Butter Rajma Chawal + Boondi Raita 🥣', price: 180, served: false },
  { id: 'rc-3', customer: 'Chandigarh Sector 17 Foodies', locality: 'Geri Route', combo: 'Desi Ghee Rajma Chawal Thali + Papad & Achaar 🌶️', price: 210, served: false },
];

export default function RajmaChawalRush() {
  const [orders, setOrders] = useState<DhabaOrder[]>(INITIAL_ORDERS);
  const [revenue, setRevenue] = useState(0);
  const [punjabiKarma, setPunjabiKarma] = useState(1050);
  ...
  const handleServeOrder = (id: string, customer: string, price: number) => {
    ...
    setPunjabiKarma(k => k + 75);
    toast.success(`🍛 Piping Hot Desi Rajma Chawal Thali served to ${customer}! (+₹${price})`);
  };

  const handleNewDhabaHandi = () => {
    sounds.playPop();
    setOrders(INITIAL_ORDERS.map(o => ({ ...o, served: false })));
    toast.info('🍲 Fresh slow-simmered Kashmiri rajma handi ready! Basmati steamed.');
  };
    ...
    <div className="min-h-screen bg-background pb-24 font-sans text-foreground">
      ...
      <div className="sticky top-0 z-30 glass-heavy px-4 py-3 sm:px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold font-display text-foreground">Desi Rajma Chawal Dhaba Express</h1>
            <p className="text-[0.68rem] text-muted-foreground font-mono">Kashmiri Red Kidney Beans, Steamed Jeera Basmati & Sirka Pyaaz</p>
          </div>
        </div>

        <Button onClick={handleNewDhabaHandi} variant="outline" className="rounded-2xl text-xs font-mono">
          <RotateCcw className="w-3.5 h-3.5 mr-1" /> New Dhaba Handi
        </Button>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 rounded-3xl surface-1 border border-border/40 text-center font-mono text-xs shadow-xl">
          ...
          <div>
            <span className="text-muted-foreground uppercase text-[0.6rem] block">Punjabi Dhaba Karma</span>
            <strong className="font-display font-black text-3xl text-red-400">+{punjabiKarma} XP</strong>
          </div>
        </div>
            ...
            {orders.map((o) => (
              ...
              >
                <div className="space-y-1">
                  <span className="text-xs font-mono text-muted-foreground">{o.locality}</span>
