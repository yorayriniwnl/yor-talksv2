...
const INITIAL_ORDERS: GheeRoastOrder[] = [
  { id: 'gr-1', customer: 'Mangalore Hampankatta Foodies', stall: 'Shetty Lunch Home Kundapura', combo: 'Fiery Byadgi Chicken Ghee Roast + Soft Lace Neer Dosas 🍗🫓', price: 440, served: false },
  { id: 'gr-2', customer: 'Udupi Krishna Math Diners', stall: 'Hotel Kidiyoor Udupi', combo: 'Fresh Malai Paneer Ghee Roast with Steamed Kotte Kadubu ✨🧀', price: 380, served: false },
  { id: 'gr-3', customer: 'Panambur Beach Weekend Guests', stall: 'Giri Manjas Mangalore', combo: 'Tiger Prawns Ghee Roast Masala with Fragrant Ghee Rice 🍤🍚', price: 480, served: false },
];

export default function MangaloreGheeRoastRush() {
  const [orders, setOrders] = useState<GheeRoastOrder[]>(INITIAL_ORDERS);
  const [revenue, setRevenue] = useState(0);
  const [tuluNaduKarma, setTuluNaduKarma] = useState(4600);
  ...
  const handleServeOrder = (id: string, customer: string, price: number) => {
    ...
    setTuluNaduKarma(k => k + 185);
    toast.success(`🍗 Sizzling Kundapura Ghee Roast served fresh to ${customer}! (+₹${price})`);
  };

  const handleLadlePureGhee = () => {
    sounds.playPop();
    setOrders(INITIAL_ORDERS.map(o => ({ ...o, served: false })));
    toast.info('🔥 Pure desi cow ghee, Byadgi chili paste & roasted Kundapura spices sizzling in the cast iron skillet!');
  };
    ...
    <div className="min-h-screen bg-background pb-24 font-sans text-foreground">
      ...
      <div className="sticky top-0 z-30 glass-heavy px-4 py-3 sm:px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold font-display text-foreground">Mangalore Ghee Roast Express</h1>
            <p className="text-[0.68rem] text-muted-foreground font-mono">Byadgi Chilies, Pure Desi Ghee Roast, Kundapura Spices & Neer Dosa</p>
          </div>
        </div>

        <Button onClick={handleLadlePureGhee} variant="outline" className="rounded-2xl text-xs font-mono">
          <RotateCcw className="w-3.5 h-3.5 mr-1" /> Sizzle Desi Ghee
        </Button>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 rounded-3xl surface-1 border border-border/40 text-center font-mono text-xs shadow-xl">
          ...
          <div>
            <span className="text-muted-foreground uppercase text-[0.6rem] block">Karavali Karma</span>
        ...
        <div className="space-y-4 font-sans">
          <div className="showcase-section-title">
            ...
            <h3>Kundapura & Mangalore Ghee Roast Orders</h3>
          </div>
              ...
              >
                ...
                {o.served ? (
                  <span className="px-4 py-2 rounded-2xl bg-emerald-500/20 text-emerald-400 font-mono font-bold text-xs flex items-center justify-center gap-1">
                    <CheckCircle2 className="w-4 h-4" /> Served Ghee Roast
                  </span>
                ) : (
                  <Button
