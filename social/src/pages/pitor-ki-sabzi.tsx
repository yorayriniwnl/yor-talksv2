...
const INITIAL_ORDERS: PitorOrder[] = [
  { id: 'pk-1', customer: 'Jodhpur Mehrangarh Royal Feast Walkers', stall: 'Gypsy Dining Hall Jodhpur', combo: 'Spiced Besan Pitor Ki Sabzi + Hot Ghee Bajra Rotla 🫓🍛', price: 380, served: false },
  { id: 'pk-2', customer: 'Bikaner Junagarh Heritage Families', stall: 'Chokhi Dhani Marwar Rasoi', combo: 'Curd Dahi Gravy Pitor + Lehsun Ki Chutney & Boondi Raita 🥗✨', price: 420, served: false },
  { id: 'pk-3', customer: 'Jaisalmer Golden Fort Travellers', stall: 'Shree Thal Village Dining', combo: 'Golden Ghee Shallow-Fried Pitor + Pulao & Sangri Sabzi 🍚🫐', price: 350, served: false },
];

export default function PitorKiSabziRush() {
  const [orders, setOrders] = useState<PitorOrder[]>(INITIAL_ORDERS);
  const [revenue, setRevenue] = useState(0);
  const [marwarKarma, setMarwarKarma] = useState(8600);
  ...
  const handleServeOrder = (id: string, customer: string, price: number) => {
    ...
    setMarwarKarma(k => k + 370);
    toast.success(`🍛 Royal Rajasthani Pitor Ki Sabzi served fresh to ${customer}! (+₹${price})`);
  };

  const handleSimmerDahiGravy = () => {
    sounds.playPop();
    setOrders(INITIAL_ORDERS.map(o => ({ ...o, served: false })));
    toast.info('🔥 Simmering curd gravy with hing, methi seeds, mustard oil, dry red chilies & shallow-fried besan pitors!');
  };
    ...
    <div className="min-h-screen bg-background pb-24 font-sans text-foreground">
      ...
      <div className="sticky top-0 z-30 glass-heavy px-4 py-3 sm:px-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold font-display text-foreground">Rajasthani Pitor Ki Sabzi Express</h1>
            <p className="text-[0.68rem] text-muted-foreground font-mono">Besan Pitor Diamond Cakes, Spiced Curd Dahi Gravy, Hing Tadka & Bajra Rotla</p>
          </div>
        </div>

        <Button onClick={handleSimmerDahiGravy} variant="outline" className="rounded-2xl text-xs font-mono">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 rounded-3xl surface-1 border border-border/40 text-center font-mono text-xs shadow-xl">
          ...
          <div>
            <span className="text-muted-foreground uppercase text-[0.6rem] block">Marwari Culinary Karma</span>
        ...
        <div className="space-y-4 font-sans">
          <div className="showcase-section-title">
            ...
            <h3>Marwar Pitor Ki Sabzi & Bajra Rotla Orders</h3>
          </div>
              ...
              >
                ...
                {o.served ? (
                  <span className="px-4 py-2 rounded-2xl bg-emerald-500/20 text-emerald-400 font-mono font-bold text-xs flex items-center justify-center gap-1">
                    <CheckCircle2 className="w-4 h-4" /> Served Pitor Sabzi
                  </span>
                ) : (
                  <Button
