...
export type OwnUserView = Omit<UserRecord, "passwordHash" | "totpSecret"> & { twoFactorEnabled: boolean };
export type PublicUserView = Omit<UserRecord, "passwordHash" | "email" | "devices" | "blockedUsers" | "mutedUsers" | "privacy" | "passwordResetRequired" | "totpSecret">;
...
export function toPublicUser(user: UserRecord): PublicUserView {
  const { passwordHash, email, devices, blockedUsers, mutedUsers, privacy, passwordResetRequired, totpSecret, ...rest } = user;
  return rest;
}
