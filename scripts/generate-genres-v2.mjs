...
const FIRST_NAMES = ['Liam', 'Emma', 'Noah', 'Olivia', 'William', 'Ava', 'James', 'Isabella', 'Oliver', 'Sophia', 'Benjamin', 'Mia', 'Elijah', 'Charlotte', 'Lucas', 'Amelia', 'Mason', 'Harper', 'Logan', 'Evelyn', 'Kenji', 'Sven', 'Tariq', 'Hiroshi', 'Zara', 'Elena', 'Mei', 'Omar', 'Kavita', 'Ravi', 'Diego', 'Mateo', 'Sofia', 'Chloe', 'Zoe', 'Jackson', 'Aiden', 'Sebastian', 'Luna', 'Mila'];
const LAST_NAMES = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez', 'Hernandez', 'Lopez', 'Gonzalez', 'Wilson', 'Anderson', 'Thomas', 'Taylor', 'Moore', 'Jackson', 'Martin', 'Yamamoto', 'Chen', 'Patel', 'Singh', 'Dubois', 'Müller', 'Rossi', 'Kim', 'Okafor', 'Ivanov', 'Silva', 'Santos', 'Russo', 'Conti', 'Ali', 'Hassan', 'Cohen', 'Levi', 'Wang', 'Li'];

function generateUsers(count) {
  let output = `\n// --- MASSIVE EXPANSION USERS ---\nObject.assign(MOCK_USERS, {\n`;
  const userIds = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const username = `${firstName.toLowerCase()}_${genre.prefix}_${randomInt(10, 9999)}`;
    ...
    bio: '${genre.label} Content Creator ${genre.emoji} Living on the cutting edge of art, culture, and technique.',
    followers: ${randomInt(100, 2000000)},
    following: ${randomInt(10, 2000)},
...
function generateVideos(userIds, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    description: 'A deep look into ${genre.label}! We just pushed the limits of what is possible here. ${genre.emoji}',
    likes: ${randomInt(500, 150000)},
    comments: ${randomInt(10, 5000)},
    shares: ${randomInt(5, 2000)},
...
function generatePosts(userIds, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    id: '${id}',
    authorId: '${authorId}',
    content: 'Just finished an incredible session focusing on ${genre.label}. The methodology changes everything! ${genre.emoji} What do you think about this setup?',
    media: [
      '${randomItem(IMAGES)}'
    ],
...
async function run() {
  const { output: usersCode, userIds } = generateUsers(150); // 150 more users
  const videosCode = generateVideos(userIds, 600); // 600 more videos
  const postsCode = generatePosts(userIds, 300); // 300 more posts
