...
requireMigrationSecret(CONTACT_SHIELD_SECRET);
...
function pushFreshSchema() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!pnpmCli || !path.basename(pnpmCli).toLowerCase().includes("pnpm")) {
      reject(new Error("Run the beta migration through pnpm so the checked-in pnpm runtime can apply the base schema"));
      return;
    }
...
async function ensureBaseSchemaBootstrap() {
  const objects = await inspectBaseSchema();
  const action = planBaseSchema(objects, REQUIRED_BASE_TABLES);
  if (action === "bootstrap") {
    console.log("[beta migration] Empty public schema detected; applying the checked-in base schema once.");
...
export async function migrate() {
  await ensureBaseSchemaBootstrap();
