...
async function ensureCreatorAnalyticsSchema() {
  ...
  // Older beta builds created this as a plain index. Replace it explicitly so
  // analytics upserts can rely on the creator/day uniqueness guarantee.
  await client.query("DROP INDEX IF EXISTS analytics_creator_date_idx");
  await client.query("CREATE UNIQUE INDEX analytics_creator_date_idx ON creator_analytics_daily (creator_id, date)");
}
