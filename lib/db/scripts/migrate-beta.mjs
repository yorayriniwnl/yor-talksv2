...
async function ensureConsentSchema() {
  console.log("[beta migration] Enabling versioned terms and age consent...");
  ...
  await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS age_confirmed_at timestamptz");
}
...
async function ensurePostAudienceSchema() {
  console.log("[beta migration] Enabling per-post audience controls...");
  await client.query("ALTER TABLE posts ADD COLUMN IF NOT EXISTS audience text DEFAULT 'public'");
  await client.query("UPDATE posts SET audience = 'public' WHERE audience IS NULL OR btrim(audience) = ''");
  await client.query("ALTER TABLE posts ALTER COLUMN audience SET DEFAULT 'public'");
  await client.query("ALTER TABLE posts ALTER COLUMN audience SET NOT NULL");
  await client.query("CREATE INDEX IF NOT EXISTS post_audience_idx ON posts (audience, created_at)");
}

async function ensurePremiumProfileSchema() {
  console.log("[beta migration] Enabling premium profile styles, icons, distribution, and pins...");
  await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS bio_style_id text NOT NULL DEFAULT 'default'");
  await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS message_font_id text NOT NULL DEFAULT 'default'");
  await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS story_font_id text NOT NULL DEFAULT 'default'");
  await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS app_icon_id text NOT NULL DEFAULT 'yor-default'");
  await client.query("ALTER TABLE posts ADD COLUMN IF NOT EXISTS distribution_mode text NOT NULL DEFAULT 'feed_and_profile'");
  await client.query("CREATE INDEX IF NOT EXISTS post_distribution_mode_idx ON posts (distribution_mode, created_at)");
  await client.query(`
    CREATE TABLE IF NOT EXISTS profile_post_pins (
  ...
  `);
  await client.query("CREATE UNIQUE INDEX IF NOT EXISTS profile_post_pin_position_idx ON profile_post_pins (user_id, position)");
  await client.query("CREATE INDEX IF NOT EXISTS profile_post_pin_post_idx ON profile_post_pins (post_id)");
}

async function ensurePremiumTypographySchema() {
  console.log("[beta migration] Enabling per-message and per-story typography...");
  await client.query("ALTER TABLE messages ADD COLUMN IF NOT EXISTS text_style_id text NOT NULL DEFAULT 'default'");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS story_font_id text NOT NULL DEFAULT 'default'");
  await client.query(`ALTER TABLE stories ADD COLUMN IF NOT EXISTS story_text_style jsonb NOT NULL DEFAULT '{"size":"md","weight":"strong","align":"center","background":"none","backgroundOpacity":0,"positionX":50,"positionY":50,"rotation":0}'::jsonb`);
}
...
async function ensureCloseFriendsSchema() {
  console.log("[beta migration] Enabling Close Friends audience controls...");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS audience text NOT NULL DEFAULT 'followers'");
  await client.query("UPDATE stories SET audience = 'followers' WHERE audience IS NULL OR btrim(audience) = ''");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS user_close_friends_friend_idx ON user_close_friends (friend_id)");
  await client.query("CREATE INDEX IF NOT EXISTS user_close_friends_user_idx ON user_close_friends (user_id, created_at)");
}

async function ensureBroadcastChannelsSchema() {
  console.log("[beta migration] Enabling creator Broadcast Channels...");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS broadcast_channel_owner_idx ON broadcast_channels (owner_id)");
  await client.query("CREATE INDEX IF NOT EXISTS broadcast_channel_created_at_idx ON broadcast_channels (created_at)");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS broadcast_channel_member_user_idx ON broadcast_channel_members (user_id, created_at)");
  await client.query(`
  ...
  `);
  await client.query("ALTER TABLE broadcast_channel_messages ADD COLUMN IF NOT EXISTS reactions jsonb NOT NULL DEFAULT '{}'::jsonb");
  await client.query("CREATE INDEX IF NOT EXISTS broadcast_channel_message_channel_idx ON broadcast_channel_messages (channel_id, created_at)");
  await client.query("CREATE INDEX IF NOT EXISTS broadcast_channel_message_author_idx ON broadcast_channel_messages (author_id)");
}

async function ensureCommunityMembershipSchema() {
  console.log("[beta migration] Normalizing community memberships...");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS community_member_user_idx ON community_members (user_id, created_at)");
  if (await columnExists("communities", "member_ids")) {
    await client.query(`
      INSERT INTO community_members (community_id, user_id, role)
      SELECT community.id, app_user.id,
        CASE WHEN app_user.id = community.owner_id THEN 'owner' ELSE 'member' END
      FROM communities community
      CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(community.member_ids, '[]'::jsonb)) member(value)
      JOIN users app_user ON app_user.id::text = member.value
      ON CONFLICT (community_id, user_id) DO NOTHING
    `);
  }
  await client.query(`
    INSERT INTO community_members (community_id, user_id, role)
    SELECT id, owner_id, 'owner' FROM communities
    ON CONFLICT (community_id, user_id) DO UPDATE SET role = 'owner'
  `);
  await client.query("ALTER TABLE communities DROP COLUMN IF EXISTS member_ids");
}

async function ensureEventRsvpSchema() {
  console.log("[beta migration] Normalizing event RSVPs...");
  await client.query(`
    CREATE TABLE IF NOT EXISTS event_rsvps (
      event_id uuid NOT NULL REFERENCES events(id) ON DELETE CASCADE,
      user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      status text NOT NULL CONSTRAINT event_rsvp_status_check CHECK (status IN ('going', 'interested')),
      created_at timestamptz NOT NULL DEFAULT NOW(),
      updated_at timestamptz NOT NULL DEFAULT NOW(),
  ...
  await client.query(`
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'event_rsvp_status_check') THEN
        ALTER TABLE event_rsvps ADD CONSTRAINT event_rsvp_status_check CHECK (status IN ('going', 'interested'));
      END IF;
    END $$
  `);
  await client.query("CREATE INDEX IF NOT EXISTS event_rsvp_user_idx ON event_rsvps (user_id, status, updated_at)");
  await client.query("CREATE INDEX IF NOT EXISTS event_rsvp_event_status_idx ON event_rsvps (event_id, status)");
  if (await columnExists("events", "attendee_ids")) {
    await client.query(`
      INSERT INTO event_rsvps (event_id, user_id, status)
      SELECT event.id, app_user.id, 'going'
      FROM events event
      CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(event.attendee_ids, '[]'::jsonb)) attendee(value)
      JOIN users app_user ON app_user.id::text = attendee.value
      ON CONFLICT (event_id, user_id) DO UPDATE SET status = 'going', updated_at = NOW()
    `);
  }
  if (await columnExists("events", "interested_ids")) {
    await client.query(`
      INSERT INTO event_rsvps (event_id, user_id, status)
      SELECT event.id, app_user.id, 'interested'
      FROM events event
      CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(event.interested_ids, '[]'::jsonb)) interested(value)
      JOIN users app_user ON app_user.id::text = interested.value
      ON CONFLICT (event_id, user_id) DO NOTHING
    `);
  }
  await client.query("ALTER TABLE events DROP COLUMN IF EXISTS attendee_ids");
  await client.query("ALTER TABLE events DROP COLUMN IF EXISTS interested_ids");
}

async function ensureProductSaveSchema() {
  console.log("[beta migration] Normalizing marketplace saves...");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS product_save_user_idx ON product_saves (user_id, created_at)");
  if (await columnExists("products", "saved_by")) {
    await client.query(`
      INSERT INTO product_saves (product_id, user_id)
      SELECT product.id, app_user.id
      FROM products product
      CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(product.saved_by, '[]'::jsonb)) saver(value)
      JOIN users app_user ON app_user.id::text = saver.value
      ON CONFLICT (product_id, user_id) DO NOTHING
    `);
  }
  await client.query("ALTER TABLE products DROP COLUMN IF EXISTS saved_by");
}

async function ensurePremiumFeatureSchema() {
  console.log("[beta migration] Enabling Yor Advanced feature entitlements and audience membership...");
  await client.query(`
  ...
  `);
  await client.query("CREATE UNIQUE INDEX IF NOT EXISTS feature_entitlement_feature_plan_idx ON feature_entitlements (feature_key, plan_key)");
  await client.query("CREATE INDEX IF NOT EXISTS feature_entitlement_active_idx ON feature_entitlements (feature_key, status, expires_at)");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS user_feature_override_active_idx ON user_feature_overrides (user_id, status, expires_at)");
  await client.query(`
    CREATE TABLE IF NOT EXISTS story_audience_members (
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS story_audience_member_user_idx ON story_audience_members (user_id, created_at)");
  await client.query(`
    CREATE TABLE IF NOT EXISTS story_audience_exclusions (
      story_id uuid NOT NULL REFERENCES stories(id) ON DELETE CASCADE,
      user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      created_at timestamptz NOT NULL DEFAULT NOW(),
      PRIMARY KEY (story_id, user_id)
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS story_audience_exclusion_user_idx ON story_audience_exclusions (user_id, created_at)");
}

async function ensurePremiumStorySchema() {
  console.log("[beta migration] Enabling durable story lifecycle, view events, and highlights...");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS highlight_owner_idx ON highlights (owner_id, updated_at)");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS published_at timestamptz");
  await client.query("UPDATE stories SET published_at = created_at WHERE published_at IS NULL");
  await client.query("ALTER TABLE stories ALTER COLUMN published_at SET DEFAULT NOW()");
  await client.query("ALTER TABLE stories ALTER COLUMN published_at SET NOT NULL");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS highlight_id uuid REFERENCES highlights(id) ON DELETE SET NULL");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS publish_mode text NOT NULL DEFAULT 'active'");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS priority_boost integer NOT NULL DEFAULT 0");
  await client.query("ALTER TABLE stories ADD COLUMN IF NOT EXISTS engagement_score integer NOT NULL DEFAULT 0");
  await client.query("CREATE INDEX IF NOT EXISTS story_active_order_idx ON stories (publish_mode, expires_at, published_at)");
  await client.query(`
  ...
  `);
  await client.query("CREATE UNIQUE INDEX IF NOT EXISTS highlight_item_position_idx ON highlight_items (highlight_id, position)");
  await client.query(`
    CREATE TABLE IF NOT EXISTS story_view_events (
      id uuid PRIMARY KEY,
      story_id uuid NOT NULL REFERENCES stories(id) ON DELETE CASCADE,
      viewer_id uuid REFERENCES users(id) ON DELETE CASCADE,
      exposure text NOT NULL DEFAULT 'identified',
      event_key text NOT NULL,
      viewed_at timestamptz NOT NULL DEFAULT NOW()
    )
  `);
  await client.query("CREATE UNIQUE INDEX IF NOT EXISTS story_view_event_idempotency_idx ON story_view_events (story_id, event_key)");
  await client.query("CREATE INDEX IF NOT EXISTS story_view_event_story_time_idx ON story_view_events (story_id, viewed_at)");
  await client.query("CREATE INDEX IF NOT EXISTS story_view_event_viewer_time_idx ON story_view_events (viewer_id, viewed_at)");
  await client.query("ALTER TABLE story_reactions ADD COLUMN IF NOT EXISTS reaction_type text NOT NULL DEFAULT 'CUSTOM'");
}

async function ensureStoryViewerInteractionSchema() {
  console.log("[beta migration] Normalizing story views and reactions...");
  await client.query(`
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS story_view_user_idx ON story_views (user_id, viewed_at)");
  await client.query(`
    CREATE TABLE IF NOT EXISTS story_reactions (
      ...
      updated_at timestamptz NOT NULL DEFAULT NOW(),
      PRIMARY KEY (story_id, user_id),
      CONSTRAINT story_reaction_emoji_check CHECK (char_length(emoji) BETWEEN 1 AND 32)
    )
  `);
  await client.query(`
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'story_reaction_emoji_check') THEN
        ALTER TABLE story_reactions ADD CONSTRAINT story_reaction_emoji_check CHECK (char_length(emoji) BETWEEN 1 AND 32);
      END IF;
    END $$
  `);
  await client.query("CREATE INDEX IF NOT EXISTS story_reaction_user_idx ON story_reactions (user_id, updated_at)");
  if (await columnExists("stories", "viewer_ids")) {
    await client.query(`
      INSERT INTO story_views (story_id, user_id)
      SELECT story.id, app_user.id
      FROM stories story
      CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(story.viewer_ids, '[]'::jsonb)) viewer(value)
      JOIN users app_user ON app_user.id::text = viewer.value
      ON CONFLICT (story_id, user_id) DO NOTHING
    `);
  }
  if (await columnExists("stories", "reactions")) {
    await client.query(`
      INSERT INTO story_reactions (story_id, user_id, emoji)
      SELECT story.id, app_user.id, left(reaction.value->>'emoji', 32)
      FROM stories story
      CROSS JOIN LATERAL jsonb_array_elements(COALESCE(story.reactions, '[]'::jsonb)) reaction(value)
      JOIN users app_user ON app_user.id::text = reaction.value->>'userId'
      WHERE char_length(COALESCE(reaction.value->>'emoji', '')) BETWEEN 1 AND 32
      ON CONFLICT (story_id, user_id) DO UPDATE SET emoji = EXCLUDED.emoji, updated_at = NOW()
    `);
  }
  await client.query("ALTER TABLE stories DROP COLUMN IF EXISTS viewer_ids");
  await client.query("ALTER TABLE stories DROP COLUMN IF EXISTS reactions");
}
...
async function ensureMessagePreviewSchema() {
  console.log("[beta migration] Enabling non-reading message previews...");
  await client.query(`
    CREATE TABLE IF NOT EXISTS message_preview_events (
  ...
  `);
  await client.query("CREATE INDEX IF NOT EXISTS message_preview_user_idx ON message_preview_events (user_id, previewed_at)");
}
...
export async function migrate() {
  ...
  try {
    ...
    await ensurePostAudienceSchema();
    await ensurePremiumProfileSchema();
    await ensurePremiumTypographySchema();
    ...
    await ensureCloseFriendsSchema();
    await ensureCommunityMembershipSchema();
    await ensureEventRsvpSchema();
    await ensureProductSaveSchema();
    await ensurePremiumFeatureSchema();
    await ensureStoryViewerInteractionSchema();
    await ensurePremiumStorySchema();
    await ensureBroadcastChannelsSchema();
    await ensureDisappearingMessagesSchema();
    await ensureMessagePreviewSchema();
